...
class GoRenameCommand(sublime_plugin.TextCommand):
    ...
    def run(self, edit, simulate=False, force=False, verbose=False, quick=False):
        ...
        try:
            ...
            sublime.error_message("GoRename:\nCouldn't get cursor position, make sure that the Go source file is saved and the cursor is over the identifier (variable, function ...) you want to query.")
        ...
        def rename_name_input(name):
            ...
            renameMe['compiled_message'] = message
        ...
        def popup_menu_callback(flag_opt):
            ...
            if flag_opt != -1:
                ...
                if get_setting("gorename_autofill", False):
                    varName = word
                self.view.window().show_input_panel('GoRename: rename "%s" (from line %s) to' %
                                                    (word, line_number), varName, rename_name_input, on_change=None, on_cancel=None)
...
class GoRenameConfirmCommand(sublime_plugin.TextCommand):
    ...
    def run(self, edit):
        ...
        if not runningTool:
            if hashlib.sha256(open(renameMe['file_path'], 'rb').read()).hexdigest() != renameMe['checksum'] and not get_setting('gorename_rename_modified_files', False):
                ...
                GR.gorename(
                    file_path=renameMe['file_path'],
                    begin_offset=renameMe['offset'],
                    name=renameMe['name'],
                    flags=renameMe['flags'],
                    callback=GR.gorename_complete,
                )
                renameMe = {}
        else:
